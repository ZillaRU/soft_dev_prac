...
public class ActivitiDemo {
    ...
    /**
     * 发布流程
     * 部署流程定义：act_re_deployment(1) act_re_procdef(1) act_ge_bytearray(2)
     */
    @Test
    public void test() {
...
//                .name("流程名：项目审批")
//                .addClasspathResource("bpmn/立项审批.bpmn20.xml")
//                .addClasspathResource("bpmn/立项审批.bpmn20.png")
//                .deploy();
//        System.out.println(deployment.getId());
        System.out.println(repositoryService.createDeploymentQuery().count());
        // 查询流程定义 act_re_procdef
        ProcessDefinitionQuery query = repositoryService.createProcessDefinitionQuery();
        // query.processDefinitionKey("processKey"); //filter
        // query.orderByDeploymentId().asc(); // sort
        // query.listPage()  // 分页
        List<ProcessDefinition> list = query.list();
        for(ProcessDefinition definition:list) {
            System.out.println("precessDefId = " + definition.getId());
        }

//        1
//        precessDefId = ProjectApply:1:27504
    }


    /**
     * 启动一个流程（流程实例）
     * act_ru_execution(1) act_ru_task(1)
     *
     * DeploymentQuery RepositoryService
     * ProcessInstanceQuery RuntimeService
     * TaskQuery TackService
     * processEngine.getXXXService.createXXXQuery
     */
    @Test
    public void startUp() {
        Map<String, Object> varMap = new HashMap<>();
        varMap.put("proj_name", "让我康康怎么用");
        varMap.put("reci_addr", "744346614@qq.com");
        varMap.put("pm_addr","562064601@qq.com");
        ProcessInstance instance = runtimeService.startProcessInstanceByKey("ProjectApply", varMap);
        System.out.println(instance.getId());
        System.out.println(runtimeService.createProcessInstanceQuery().list());

//        30001
//        [ProcessInstance[30001]]
...
//     .processDefinitionName("Myleave");//通过name获取
   ...
   /* ProcessInstance pi = runtimeService//表示正在执行的流程实例和执行对象
    ...
    public void image() throws Exception {
        ...
        byte[] b = new byte[1024];
        int len;
        FileOutputStream outputStream = new FileOutputStream("../logs/testProcessImg/1.png");
